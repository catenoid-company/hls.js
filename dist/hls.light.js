...
var level_controller_LevelController = /*#__PURE__*/function (_EventHandler) {
  ...
  function LevelController(hls) {
    ...
    _this.timer = null;
    _this._startBitrateLevel = hls.config.startBitrateLevel || 0;
  ...
  _proto.onManifestLoaded = function onManifestLoaded(data) {
    ...
    if (levels.length > 0) {
      ...
      this._levels = levels; //Sort 이후 startBitrateLevel에 해당하는 bitrateStart값을 설정한다.

      bitrateStart = levels[this._startBitrateLevel].bitrate; // find index of first level in sorted levels
